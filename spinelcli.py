...
#configuracion credenciales datacake
...
def on_message(client, userdata, message):
    ...
    A = A[2:-1]
    if A == "ID":
        os.system("wisunstack start")
        print("SOLICITUD DE ID")
        # instruccion a pyserial con comando 
    elif A  == "TIME":
        print("SOLICITA TIEMPO")
        # instruccion a pyserial con comando
    elif A  == "CALI":
        print("SOLICITA CALIBRACION")
        # instruccion a pyserial con comando
    elif A  == "CORTE":
        print("SOLICITUD DE CORTE")
        # instruccion a pyserial con comando
    elif A  == "FACTURACION":
        print("SOLICITUD DE FACTURACION")
        # instruccion a pyserial con comando
    else:
        b = A[0]
        print(b)
        A = A[1:]
        if b == "t": # PARA TIEMPO
            print("ajuste tiempo "+A)
        elif b == "b": #PARA BETA
            print("ajuste beta"+A)
        elif b == "i": #PARA CALIBACION
            print("ajuste calibracion"+A)    
...
# CoAP Consumo reportado
COAP_UPDATE_CONSUMO_URI = "upd_consumo"
COAP_CALIBRACION_URI = "upd_calibracion"
COAP_TIEMPO_URI = "upd_tiempo"
COAP_ID_UPD_URI = "upd_id"
COAP_BETA_UPD_URI = "upd_beta"
COAP_CORTE_UPD_URI = "upd_corte"
COAP_FACTURACION_URI = "facturacion"
...
class IPv6Factory(object):
    ...
    def build_coap_request(self, src, dst, coap_type, coap_method_code, uri_path, option_list=None,
        ...
        coap_payload = None
        if byte0 is not None and byte1 is not None and byte2 is not None and byte3 is not None and byte4 is not None and byte5 is not None:
            coap_payload = ipv6.CoAPPayload(bytes([byte0, byte1, byte2, byte3, byte4, byte5]))
                ...
                if pkt.upper_layer_protocol.type == ipv6.IPV6_NEXT_HEADER_ICMP:
                    ...
                    if udp_pkt.header.dst_port == COAP_PORT:
                        ...
                        if h.type == ipv6.COAP_TYPE_ACK:
                            if oad_fwv_req_token == h.token and h.code == ipv6.COAP_RSP_CODE_CONTENT:
                                ...
                                if h.code == ipv6.COAP_RSP_CODE_CONTENT:
                                    coap_packet_type = "WC_GET_RESP"
                                elif h.code == ipv6.COAP_RSP_CODE_CHANGED:
                                    coap_packet_type = "WC_SET_RESP"
                            ...
                            for option in h.options:
                                if option.option_num == ipv6.COAP_OPTION_URI_PATH:
                                    ...
                                    elif(option.option_val == str.encode(COAP_UPDATE_CONSUMO_URI)):
                                        # Handle ev charger CoAP message
                                        # The coap message type is a coap post request.
                                        coap_packet_type = "WHALECOMM_CONSUMO_UPDATE"
                                    elif(option.option_val == str.encode(COAP_CALIBRACION_URI)):
                                        # Handle ev charger CoAP message
                                        # The coap message type is a coap post request.
                                        coap_packet_type = "WHALECOMM_CALIBRACION"
                                    elif(option.option_val == str.encode(COAP_FACTURACION_URI)):
                                        # Handle ev charger CoAP message
                                        # The coap message type is a coap post request.
                                        coap_packet_type = "WHALECOMM_FACTURACION"
                                    elif(option.option_val == str.encode(COAP_TIEMPO_URI)):
                                        # Handle ev charger CoAP message
                                        # The coap message type is a coap post request.
                                        coap_packet_type = "WHALECOMM_TIEMPO"
                                    elif(option.option_val == str.encode(COAP_ID_UPD_URI)):
                                        # Handle ev charger CoAP message
                                        # The coap message type is a coap post request.
                                        coap_packet_type = "WHALECOMM_ID"
                                    elif(option.option_val == str.encode(COAP_BETA_UPD_URI)):
                                        # Handle ev charger CoAP message
                                        # The coap message type is a coap post request.
                                        coap_packet_type = "WHALECOMM_BETA"
                                    elif(option.option_val == str.encode(COAP_CORTE_UPD_URI)):
                                        # Handle ev charger CoAP message
                                        # The coap message type is a coap post request.
                                        coap_packet_type = "WHALECOMM_CORTE"
                                    else:
                        ...
                        # OAD abort request
                            ...
                            return
                        # LED GET response
                        elif coap_packet_type == "WC_GET_RESP":
                            if len(p.payload) == 2:
                                ...
                                print("RLED state: {}, GLED state: {}".format(p.payload[0], p.payload[1]))
                            #elif len(p.payload) == 6:
                                #print("Respuesta medidor: {},{},{},{},{},{}".format(p.payload[0], p.payload[1], p.payload[2], p.payload[3], p.payload[4], p.payload[5]))
                                #print("Error: invalid LED state")
                            #elif len(p.payload) == 3:
                                #print("Respuesta medidor: {},{},{}".format(p.payload[0], p.payload[1], p.payload[2]))
                           # else:
                               # print("Respuesta fuera de rango...")
                        # LED PUT/POST response
                        elif coap_packet_type == "WC_SET_RESP":
                            print("Confirmacion de asignación de parámetros...")
                        ...
                        elif(coap_packet_type == "WHALECOMM_CONSUMO_UPDATE"):
                            # Handle ev charger CoAP message
                            if h.code == ipv6.COAP_METHOD_CODE_POST:
                                ...
                                if h.type == ipv6.COAP_TYPE_CON:
                                    try:
                                        ...
                                        print(traceback.format_exc())
                        elif(coap_packet_type == "WHALECOMM_CALIBRACION"):
                            # Handle ev charger CoAP message
                            if h.code == ipv6.COAP_METHOD_CODE_POST:
                                ...
                                if h.type == ipv6.COAP_TYPE_CON:
                                    try:
                                        ...
                                        print("Failed to send acknowledgement")
                                        print(traceback.format_exc())
                        elif(coap_packet_type == "WHALECOMM_FACTURACION"):
                            # Handle ev charger CoAP message
                            if h.code == ipv6.COAP_METHOD_CODE_POST:
                                ...
                                if h.type == ipv6.COAP_TYPE_CON:
                                    try:
                                        ...
                                        print("Failed to send acknowledgement")
                                        print(traceback.format_exc())
                        elif(coap_packet_type == "WHALECOMM_TIEMPO"):
                            # Handle ev charger CoAP message
                            if h.code == ipv6.COAP_METHOD_CODE_POST:
                                ...
                                if h.type == ipv6.COAP_TYPE_CON:
                                    try:
                                        ...
                                        print("Failed to send acknowledgement")
                                        print(traceback.format_exc())
                        elif(coap_packet_type == "WHALECOMM_ID"):
                            # Handle ev charger CoAP message
                            if h.code == ipv6.COAP_METHOD_CODE_POST:
                                ...
                                if h.type == ipv6.COAP_TYPE_CON:
                                    try:
                                        ...
                                        print("Failed to send acknowledgement")
                                        print(traceback.format_exc())
                        elif(coap_packet_type == "WHALECOMM_BETA"):
                            # Handle ev charger CoAP message
                            if h.code == ipv6.COAP_METHOD_CODE_POST:
                                ...
                                if h.type == ipv6.COAP_TYPE_CON:
                                    try:
                                        ...
                                        print("Failed to send acknowledgement")
                                        print(traceback.format_exc())
                        elif(coap_packet_type == "WHALECOMM_CORTE"):
                            # Handle ev charger CoAP message
                            if h.code == ipv6.COAP_METHOD_CODE_POST:
        ...
        try:
                ...
                for i in range(4, len(params)):
                    ...
                    if params[i] == '--led_state' and len(params) >= (i + 2):
                        if params[i+1] == 'r':
                            ...
                            print("Invalid  target, must be g or r")
                            return
                        ...
                        if params[i+2] == '0':
                            ...
                            return
                    if params[i] == '--calibracion' and len(params) >= (i + 2):
                        if params[i+1] == 'c':
                            par_0 = 0
                            par_1 = 0
                            par_2 = 0
                            ...
                            print("Invalid calibration target")
                            return
                    if params[i] == '--id' and len(params) >= (i + 2):
                        if params[i+1] == 'c':
                            par_0 = ord(params[i+2])
                            par_1 = ord(params[i+3])
                            par_2 = ord(params[i+4])
                            par_3 = ord(params[i+5])
                            par_4 = ord(params[i+6])
                            par_5 = ord(params[i+7])
                        else:
                            print("Invalid calibration target")
                            return
                    if params[i] == '--beta' and len(params) >= (i + 2):
                        if params[i+1] == 'c':
                            par_0 = int(params[i+2])
                            par_1 = int(params[i+3])
                            par_2 = int(params[i+4])
                            par_3 = int(params[i+5])
                            par_4 = int(params[i+6])
                            par_5 = int(params[i+7])
                        else:
                            print("Invalid calibration target")
                            return
                    if params[i] == '--tiempo' and len(params) >= (i + 2):
                        if params[i+1] == 'c':
                            par_0 = int(params[i+2])
                            par_1 = int(params[i+3])
                            par_2 = int(params[i+4])
                            ...
                            print("Invalid calibration target")
                            return
                    if params[i] == '--corte' and len(params) >= (i + 2):
                        if params[i+1] == 'c':
                            par_0 = 0
                            par_1 = 0 
                            par_2 = 0
                            ...
                            print("Invalid calibration target")
                            return
                    if params[i] == '--facturacion' and len(params) >= (i + 2):
                        if params[i+1] == 'c':
                            par_0 = int(params[i+2])
                            par_1 = 0 
                            par_2 = 0
                        ...
                        #     led_state = 0
                        # else:
                        #     print("Invalid calibration argument, must be 0")
                        #     return
            ...
            if params[1] == "get":
                coap_req = self.ipv6_factory.build_coap_request(srcIPAddress, addr, coap_confirm,
                    ipv6.COAP_METHOD_CODE_GET, uri_path, option_list, par_0, par_1, par_2, par_3, par_4, par_5,
...
pub=threading.Thread(target=subida)
sub.start()
pub.start()
